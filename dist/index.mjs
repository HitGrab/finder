import { createContext, useContext, useImperativeHandle, useMemo, useRef, useState } from "react";
import { jsx } from "react/jsx-runtime";
import { clamp, debounce, groupBy, isEqual, orderBy, range, uniqBy } from "lodash";
...
const FinderCoreContext = createContext(null);
...
function FinderContentLoading({ children: Component }) {
	...
	if (finder.isLoading) {
		if (typeof Component === "function") return /* @__PURE__ */ jsx(Component, {
...
function FinderContent({ children: renderProps }) {
	...
	return [
		renderProps.loading && /* @__PURE__ */ jsx(FinderContentLoading, { children: renderProps.loading }, "loading"),
		renderProps.empty && /* @__PURE__ */ jsx(FinderContentEmpty, { children: renderProps.empty }, "empty"),
		renderProps.noMatches && /* @__PURE__ */ jsx(FinderContentNoMatches, { children: renderProps.noMatches }, "noMatches"),
		renderProps.items && /* @__PURE__ */ jsx(FinderContentItems, { children: renderProps.items }, "items"),
		renderProps.groups && /* @__PURE__ */ jsx(FinderContentGroups, { children: renderProps.groups }, "groups")
	];
}
...
function StringMatch({ needle, haystack, Match = "mark", Miss }) {
	...
	return segments.map((segment, index) => {
		...
		if (segment.is_match) {
			if (typeof Match === "string") return /* @__PURE__ */ jsx(Match, {
				"data-is-match": segment.is_match,
				children: segment.value
			}, key);
			return /* @__PURE__ */ jsx(Match, {
				"data-is-match": segment.is_match,
				segment,
				segmentIndex: index
			}, key);
		} else if (Miss !== void 0) {
			if (typeof Miss === "string") return /* @__PURE__ */ jsx(Miss, {
				"data-is-match": segment.is_match,
				children: segment.value
			}, key);
			return /* @__PURE__ */ jsx(Miss, {
				"data-is-match": segment.is_match,
				segment,
				segmentIndex: index
			}, key);
...
function FinderSearchTermHaystack({ Match = "mark", Miss, children: haystack }) {
	...
	return /* @__PURE__ */ jsx(StringMatch, {
...
function DebounceCallbackRegistry() {
	...
	return (rule, callback) => {
		...
		if (registry.has(rule) === false) registry.set(rule, debounce((cb) => cb(), rule.debounceMilliseconds));
...
var GroupByMixin = class {
	...
	static process(options, items, context) {
		...
		if (orderByCallbacks.length > 0) return orderBy(groups, orderByCallbacks, orderSortDirection);
		return groups;
	}
};
/**
...
var PaginationMixin = class {
	...
	get page() {
		if (this.numItemsPerPage && this.lastPage) return clamp(this.#page, 1, this.lastPage);
		return this.#page;
	}
	get offset() {
		if (this.numItemsPerPage && this.lastPage) return (clamp(this.#page, 1, this.lastPage) - 1) * this.numItemsPerPage;
		return 0;
	}
	serialize() {
		return {
	...
	static process(options, items) {
		...
		const offset = (clamp(options.page, 1, lastPage) - 1) * options.numItemsPerPage;
...
function defaultSearchAndSortAlgorithm(options, items, context) {
	const matches = items.reduce((acc, item) => {
		...
		if (itemHaystackScores.length > 0) {
			const bestScore = orderBy(itemHaystackScores, ["percentOfHaystackMatched", "longestSequentialSequence"], ["desc", "asc"]).at(0);
...
function Finder({ items, rules, effects, initialSearchTerm, initialSortBy, initialSortDirection, initialGroupBy, initialGroupBySortDirection, initialFilters, context, isLoading, disabled, page, numItemsPerPage, requireGroup, ignoreSortByRulesWhileSearchRuleIsActive, onInit, onReady, onFirstUserInteraction, onChange, controllerRef, children }) {
	...
	return /* @__PURE__ */ jsx(FinderCoreContext.Provider, {
		value,
		children
	});
}
...
//#endregion
export { Finder, StringMatch, filterRule, finderRuleset, groupByRule, ruleEffect, searchEffect, searchRule, sortByRule, transformFilterToBoolean, transformFilterToMultiple, transformFilterToSingleValue, useFinder, useFinderRef };